...
public struct FileUploadView: View, KeyboardReadable {
    ...
    var areNamesValid: Bool {
        guard let modelParentFolder = model.parentFolder else { return false }
        return model.itemsLocalNameAndPath.allSatisfy { FilenameValidator.isNameValid(ownerLogin: modelParentFolder.ownerLogin, storage: modelParentFolder.storageTypeString, prefix: modelParentFolder.prefix, name: $0.itemName) }
    }
    ...
    public var body: some View {
        if let modelParentFolder = model.parentFolder {
            GeometryReader { g in
                VStack(spacing: 0) {
                    ...
                    List {
                        ForEach(model.itemsLocalNameAndPath.indices, id: \.self) { idx in
                            ...
                            VStack{
                                ...
                                if let uploadingItemID = model.itemsUploading[currentItem.itemURL], var progress = model.itemsUploadProgress[uploadingItemID] {
                    ...
                    HStack {
                        ...
                        .disabled(self.areNamesValid == false || model.isUploading == true)
                    }
                    .padding()
                }
            }
            .sheet(isPresented: $isFolderChooserPresented, content: {
                FolderChooser(routeData: FCRouteData(folder: modelParentFolder), folderResult: $model.parentFolder)
            })
